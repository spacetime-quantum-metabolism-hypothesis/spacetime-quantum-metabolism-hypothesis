...
CLASS porting: translate functions below to source/sqmh_background.c.
Signature convention:
  double V_sqmh(double phi, int V_family, double * V_params);
  double dVdphi_sqmh(double phi, int V_family, double * V_params);

Python here serves as verification oracle for verify_background.py.
"""
import numpy as np

# V family codes (must match C header)
V_MASS = 0
V_RP = 1
V_EXP = 2
...
if __name__ == "__main__":
    ...
    print("V_mass(phi=1, A=1.37):", V_sqmh(1.0, V_MASS, [1.37]))
    print("  dV/dphi:", dVdphi_sqmh(1.0, V_MASS, [1.37]))

    # V_RP: A = Omega_DE_0
    print("V_RP(phi=1, A=0.685, n=1):", V_sqmh(1.0, V_RP, [0.685, 1.0]))
    print("  dV/dphi:", dVdphi_sqmh(1.0, V_RP, [0.685, 1.0]))

    # V_exp: A = Omega_DE_0 * exp(lam=1)
    print("V_exp(phi=1, A=0.685*e, lam=1):",
          V_sqmh(1.0, V_EXP, [0.685 * np.e, 1.0]))
    print("  dV/dphi:",
          dVdphi_sqmh(1.0, V_EXP, [0.685 * np.e, 1.0]))
