# -*- coding: utf-8 -*-
"""
L4 C27 MCMC driver: Deser-Woodard non-local gravity.
Produces result.json with posterior, CPL extraction, kill-criteria flags.
"""
...
from common import (LCDM_CHI2, LCDM_OM, LCDM_H, OMEGA_R,
                    ...
                    growth_fs8, cassini_ok)  # noqa: E402
from C27.background import build_E  # noqa: E402
from C27.perturbation import (mu_of_a_factory, sound_speed_sq,
...
# theta = (c0, X_shift, dX)
THETA_BOUNDS = [(-0.5, 0.5), (0.3, 2.5), (0.05, 1.0)]
THETA0 = [-0.15, 0.95, 0.15]
NAMES = ['Om', 'h', 'c0', 'X_shift', 'dX']


def log_prob(x):
    Om, h, c0, X_shift, dX = x
    if not (0.28 <= Om <= 0.36):
        return -np.inf
    if not (0.64 <= h <= 0.71):
        return -np.inf
...
def main():
    np.random.seed(42)
    # Fast seed fit: use LCDM best-fit + L3 theta ballpark as initial point,
    # then run a short scipy minimize with capped iterations (tight_fit too slow).
    print("[C27] seed fit (capped Nelder-Mead) ...", flush=True)
    from scipy.optimize import minimize
    x_init = [LCDM_OM, LCDM_H, -0.10, 0.95, 0.15]
    ...
    r = minimize(nll, x_init, method='Nelder-Mead',
                 options={'maxiter': 200, 'xatol': 1e-3, 'fatol': 1e-2,
                          'adaptive': True})
    x0 = list(r.x)
    print(f"[C27]   seed chi2={r.fun:.3f}", flush=True)
    print(f"[C27]   seed x0={x0}", flush=True)
    print("[C27] Running MCMC (nwalkers=24, nsteps=400, nburn=100) ...")
    mcmc = run_mcmc(log_prob, x0, NAMES,
                    nwalkers=24, nsteps=400, nburn=100, seed=42)
    means = mcmc['means']
    stds = mcmc['stds']
    rhat = mcmc['rhat']
    print(f"[C27]   backend={mcmc['backend']}  rhat={rhat}")
    for n, m, s in zip(NAMES, means, stds):
        print(f"[C27]   {n}: {m:.5f} +/- {s:.5f}")

    # max posterior from samples
    ...
    cs2_min = sound_speed_sq(E_m.payload, *theta_m)
    ...
    # L3 toy
    with open(os.path.join(os.path.dirname(_L4), 'l3', 'results', 'C27.json'),
    ...
    # Q flags
    def in_2sigma_lcdm(mean, std, val):
        if std <= 0:
            return False
        return abs(mean - val) < 2.0 * std

    lcdm_in_2sigma = (in_2sigma_lcdm(means[0], stds[0], LCDM_OM) and
                      in_2sigma_lcdm(means[1], stds[1], LCDM_H) and
                      all(abs(means[2 + i]) < 2.0 * max(stds[2 + i], 1e-4)
                          for i in range(0)))  # only c0 would tag LCDM
    # LCDM-like: c0 ~ 0
    c0_excludes_0 = abs(means[2]) > 2.0 * max(stds[2], 1e-4)
    if (not c0_excludes_0) and delta_chi2 > -6.0:
        killed.append('K12')

    Q = {
        'Q1_all_K_clear': len(killed) == 0,
        'Q2_posterior_excludes_lcdm_or_dchi2_le_-6':
            bool(c0_excludes_0 or delta_chi2 <= -6.0),
    ...
    result = {
        'ID': 'C27',
        'name': 'Deser-Woodard non-local f(box^-1 R)',
        'family': 'Non-local',
        'Om': Om_m, 'h': h_m, 'theta': theta_m,
        'theta_names': ['c0', 'X_shift', 'dX'],
        ...
        'keep': bool(keep),
        'Q_flags': Q,
        'notes': ('Localised 4-ODE (U, V) self-consistently iterated with '
                  'modified Friedmann. Cassini gamma-1=0 exact via R=0 in '
                  'Schwarzschild (Koivisto 2008).'),
    }

    out = os.path.join(_HERE, 'result.json')
    with open(out, 'w', encoding='utf-8') as f:
        json.dump(result, f, indent=2, ensure_ascii=False)
    print(f"[C27] wrote {out}")
    ...
    print(f"[C27] w0={w0:.4f}  wa={wa:.4f}  delta_chi2={delta_chi2:+.3f}  keep={keep}")
