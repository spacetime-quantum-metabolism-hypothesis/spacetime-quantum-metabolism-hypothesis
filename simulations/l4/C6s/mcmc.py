# -*- coding: utf-8 -*-
"""
L4 C6s MCMC driver (Stringy RVM + Chern-Simons).

Background identical to C5r; parameter name nu -> nu_s.
...
CAND = 'C6s'
NAME = 'Stringy RVM + Chern-Simons anomaly'
FAMILY = 'RVM'
THETA_NAMES = ['nu_s']
...
def log_prob(x):
    Om, h, nu_s = float(x[0]), float(x[1]), float(x[2])
...
def main():
    ...
    print(f"[{CAND}] best-fit Om={Om_bf:.4f} h={h_bf:.4f} nu_s={nu_bf:+.5f}")
    ...
    print(f"[{CAND}] nu_s 68% = [{nu_1sigma_lo:+.5f}, {nu_1sigma_hi:+.5f}]")
    print(f"[{CAND}] nu_s 95% = [{nu_2sigma_lo:+.5f}, {nu_2sigma_hi:+.5f}]")

    post = {
        'candidate': CAND, 'name': NAME, 'backend': mcmc['backend'],
        'param_names': ['Omega_m', 'h', 'nu_s'],
        'nwalkers': nwalkers, 'nsteps': nsteps, 'nburn': nburn, 'seed': 42,
        'n_samples': int(len(samples)),
        'means': means, 'stds': stds, 'rhat': rhat,
        'nu_s_median': nu_median,
        'nu_s_1sigma': [nu_1sigma_lo, nu_1sigma_hi],
        'nu_s_2sigma': [nu_2sigma_lo, nu_2sigma_hi],
        'prior_bounds': [prior_lo, prior_hi],
        'posterior_at_boundary': posterior_at_boundary,
        'best_fit': {
            'Omega_m': Om_bf, 'h': h_bf, 'nu_s': nu_bf,
    ...
    result = {
        ...
        'verdict': (
            (
                'Posterior pinned to prior boundary (nu_s = +0.01 upper edge); '
                'unconstrained data prefers nu_s > 0 which VIOLATES SQMH sign '
                'condition. Wall-pinned 2sigma artificially excludes LCDM. '
                'SQMH-branch (nu_s<=0) effectively ruled out.'
            ) if posterior_at_boundary else
            ('LCDM-indistinguishable (K12 marginal KILL; posterior contains nu_s=0 at 2 sigma)'
             if k12_lcdm_contained else
             'LCDM excluded at 2 sigma')
        ),
        'notes': (
            'Stringy RVM + Chern-Simons anomaly. Background identical to C5r. '
            '(CS Pontryagin vanishes on FLRW/Type-D.) gamma-1=0 exact. Theory '
            'score 5. Same wall-pinning as C5r on the SQMH prior.'
        ),
    }
