...
"""L6: A08 alt-20 evidence at nlive=800."""
...
def _jsonify(obj):
    ...
    return obj
_,f_ratio=ALT['A08']; _build=_make_E(f_ratio)
...
def main():
    t0=time.time()
    print('[L6 A08-hires] nlive=800 seed=42',flush=True)
    ...
    print('[L6 A08-hires] logZ=%.4f delta=%.4f jeffreys=%s K17=%s dt=%.1fmin'%(logz,delta_logz,jeffreys,k17,dt/60),flush=True)
    result={'ID':'A08','phase':'L6-E-hires','logz':logz,'logz_err':logz_err,'lcdm_logz':_LCDM_LOGZ,'delta_logz':delta_logz,'jeffreys':jeffreys,'k17_pass':k17,'nlive':800,'niter':int(res.niter),'wall_sec':float(dt),'l5_delta_logz':9.635}
    with open(os.path.join(_HERE,'evidence_A08_hires.json'),'w',encoding='utf-8') as f: json.dump(_jsonify(result),f,indent=2)
    print('[L6 A08-hires] done',flush=True)
